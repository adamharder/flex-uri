...
# This is just a passthrough for reading a HTTPS endpoint

def scheme():
    return "https"

def factory_build(parsed_uri:ParsedUri):
    return HttpsUri.wrap(uri=parsed_uri)


@dataclass
class HttpsUri(FlexUriBase):
    uri:ParsedUri
    ...
    @staticmethod
    def wrap(uri=Union[ParsedUri, str])->"HttpsUri":
