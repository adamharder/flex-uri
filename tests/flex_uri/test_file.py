from flexuri.util_file_uri import FileUri
...
def test_bad():
    with pytest.raises(Exception):
        FileUri(uri=f"file://{get_test_res().absolute()}/y.txt").is_file()
    with pytest.raises(Exception):
        FileUri(uri="https://x.com??").is_file()


def test_file_uri():
    print(get_test_res())
    x = get_test_res()/"x.txt"


    f=FileUri(f"file://{x.absolute()}")
    ...
    assert FileUri("file:///").parent==FileUri("file:///")
    assert FileUri("file:///xx/yy").parent==FileUri("file:///xx")
    assert FileUri("file:///xx/yy.txt").stem=="yy"
    assert FileUri("file:///xx/yy.txt").suffix==".txt"
    assert FileUri("file:///xx/yy.txt").name=="yy.txt"
    assert FileUri("file:///").name == ""
    assert FileUri("file:///xx/yy.txt") == FileUri("file:///xx/yy.txt")
    assert FileUri("file:///xx/yy.txt") != FileUri("file:///xx/yy.tx")
"""
